...
namespace DW3ArduinoEditor.SaveData
{
   public class GameDataGenerator
   {
      ...
      private void WriteTablesHeader( string headerPath )
      {
         using FileStream fs = File.Create( headerPath );

         WriteToFileStream( fs, Constants.GeneratedFileMessage );
         WriteToFileStream( fs, string.Format( "#if !defined( GEN_{0}_H )\n", _gameSaveData?.HeaderGuids.TablesHeaderGuid ) );
         WriteToFileStream( fs, string.Format( "#define GEN_{0}_H\n\n", _gameSaveData?.HeaderGuids.TablesHeaderGuid ) );

         WriteToFileStream( fs, "#include \"common.h\"\n\n" );
         WriteToFileStream( fs, string.Format( "#define TABLE_CLASS_EXP_TABLE_SIZE {0}\n\n", Constants.PlayerMaxLevel - 1 ) );

         WriteToFileStream( fs, string.Format( "extern const u32 g_playerClassExpTables[8][TABLE_CLASS_EXP_TABLE_SIZE];\n\n", PlayerClass.Count ) );

         WriteToFileStream( fs, string.Format( "#endif // GEN_{0}_H\n", _gameSaveData?.HeaderGuids.MapTilesHeaderGuid ) );
      }

      private void WriteTablesSource( string sourcePath )
      {
         using FileStream fs = File.Create( sourcePath );

         WriteToFileStream( fs, Constants.GeneratedFileMessage );
         WriteToFileStream( fs, string.Format( "#include \"{0}\"\n\n", Constants.GameDataTablesHeaderFileName ) );

         WriteToFileStream( fs, string.Format( "const u32 g_playerClassExpTables[{0}][TABLE_CLASS_EXP_TABLE_SIZE] = {{\n", (int)PlayerClass.Count ) );

         for( int i = 0; i < (int)PlayerClass.Count; i++ )
         {
            WriteToFileStream( fs, "   { " );

            for ( int j = 0; j < _gameSaveData?.PlayerClassExpTables[i].ExpTable.Count; j++ )
            {
               WriteToFileStream( fs, string.Format( "{0}", _gameSaveData?.PlayerClassExpTables[i].ExpTable[j].ToString() ) );

               if ( j < _gameSaveData?.PlayerClassExpTables[i].ExpTable.Count - 1 )
               {
                  WriteToFileStream( fs, "," );
               }
      ...
      private void WriteHelperFunctions( FileStream fs )
      {
         ...
         WriteToFileStream( fs, "{\n" );
         WriteToFileStream( fs, "   tm->tilesX = tx; tm->tilesY = ty; tm->staticSpriteCount = ssc; tm->activeSpriteCount = asc; tm->portalCount = pc, tm->entityCount = ec; tm->npcCount = nc; tm->shipTileIndex = sti; tm->ramiaTileIndex = rti;\n" );
         WriteToFileStream( fs, "}\n" );
      ...
      private void WriteTileMapFunctions( FileStream fs )
      {
         ...
         for ( int i = 0; i < _gameSaveData?.TileMaps.Count; i++ )
         {
            ...
            WriteToFileStream( fs, string.Format( "         TileMap_LoadInitialData( tileMap, {0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}, {8} );\n",
               ...
               _gameSaveData.TileMaps[i].ShipDefaultTileIndex,
               _gameSaveData.TileMaps[i].RamiaDefaultTileIndex ) );

            var flags = (UInt16)(
               0 |
               ( Convert.ToInt32( _gameSaveData?.TileMaps[i].Wraps ) << 0 ) |
               ( Convert.ToInt32( _gameSaveData?.TileMaps[i].AffectsDaylight ) << 1 ) |
               ( Convert.ToInt32( _gameSaveData?.TileMaps[i].IsUnderground ) << 2 ) |
               ( Convert.ToInt32( _gameSaveData?.TileMaps[i].HasEncounters ) << 3 ) |
               ( Convert.ToInt32( _gameSaveData?.TileMaps[i].EdgePortal is not null ) << 4 ) |
               ( Convert.ToInt32( _gameSaveData?.TileMaps[i].AllowsShip ) << 5 ) |
               ( Convert.ToInt32( _gameSaveData?.TileMaps[i].AllowsRamia ) << 6 )
            );

            // bits 7 and 8 are "on ship" and "on ramia" respectively, these should be preserved
            WriteToFileStream( fs, string.Format( "         tileMap->flags = ( ( ( ( tileMap->flags >> 7 ) & 0x3 ) << 0x7 ) | 0x{0} );\n", flags.ToString( "X4" ) ) );
      ...
      {
         // TODO: a bunch of this will probably move somewhere else eventually
         ...
         WriteToFileStream( fs, string.Format( "   game->tileMap.shipTileIndex = {0};\n", 56232 ) );
         WriteToFileStream( fs, "   TileMap_CenterEntityOnTile( &game->tileMap, &game->tileMap.shipEntity, game->tileMap.shipTileIndex );\n\n");
         ...
         WriteToFileStream( fs, string.Format( "   game->tileMap.ramiaTileIndex = {0};\n", 56234 ) );
         WriteToFileStream( fs, "   TileMap_CenterEntityOnTile( &game->tileMap, &game->tileMap.ramiaEntity, game->tileMap.ramiaTileIndex );\n\n" );

         WriteToFileStream( fs, "   for ( i = 0; i < MAX_PLAYERS; i++ )\n" );
         WriteToFileStream( fs, "   {\n" );
         WriteToFileStream( fs, string.Format( "      ActiveSprite_SetDirection( game->players[i].entity->sprite, Direction_{0} );\n", _gameSaveData?.GameStartup.PlayerStartDirection.ToString() ) );
         WriteToFileStream( fs, "   }\n\n" );
